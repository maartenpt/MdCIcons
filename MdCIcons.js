		...
		if ( parse['type'] == 'sign' ) {
			...
			if ( gard2hex[basegard] && useunicode ) {
				if ( !gardlist[basegard] ) {
					...
					defs = "";
					cssurl = "https://quest.ms.mff.cuni.cz/teitok-dev/teitok/jsesh/MdCIcons.css";
					// defs = "<defs><style type=\"text/css\">@import url('"+cssurl+"');</style></defs>";
					gsvgtxt = "<svg xmlns=\"http://www.w3.org/2000/svg\" maxw=\""+(lineheight/2)+"\">"+defs+"<text y=\""+(textheight)+"px\" class=\"sign\" font-size=\""+textheight+"px\">"+unicode+"</text></svg>";
...
// 						if ( gardnr.substr(-1) == "1" ) { count = 90; };
	...
	function makeunknownsvg(gardnr) {
		 gsvgtxt = "<svg xmlns=\"http://www.w3.org/2000/svg\" maxw=\""+(lineheight/2)+"\"><text fill=\"red\" y=\""+(textheight)+"px\" font-size=\""+textheight+"px\">"+gardnr+"</text></svg>";
